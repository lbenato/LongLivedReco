...
parser.add_option("-l", "--lists", action="store", type="string", dest="lists", default="reconstruction")
...
elif options.lists == "reconstruction":
    from crab_requests_lists_reconstruction import *
    crabConfig = 'crabConfig_reconstruction.py'
    folder = "reconstruction_v2_4May2018"
else:
...
for a, j in enumerate(requests):
    if options.crabaction=="submit":
        ...
        if options.lists == "reconstruction":#reclustered jets dataset are all in phys03
            os.system('echo submitting... ' + crabConfig + ' with options...\n' + 'echo General.requestName=' + j + ' General.workArea=' + folder + ' Data.outLFNDirBase=/store/user/lbenato/' + folder + ' Data.inputDataset=' + requests[j] + ' Data.inputDBS="phys03" '  + '\n')
            os.system('crab submit -c ' + crabConfig + ' General.workArea=' + folder + ' Data.outLFNDirBase=/store/user/lbenato/' + folder + ' General.requestName=' + j + ' Data.inputDataset=' + requests[j] + ' Data.inputDBS="phys03" '  + '\n')
        else:
