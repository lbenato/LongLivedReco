...
JetAnalyzer::JetAnalyzer(edm::ParameterSet& PSet, edm::ConsumesCollector&& CColl):
  //RecoJetToken(CColl.consumes<std::vector<reco::Jet> >(PSet.getParameter<edm::InputTag>("recoJets"))),
  //RecoMetToken(CColl.consumes<std::vector<reco::PFMET> >(PSet.getParameter<edm::InputTag>("recoMet"))),
...
//////////////////////////////////////////////
/*
std::vector<reco::PFJet> JetAnalyzer::FillRecoJetVector(const edm::Event& iEvent) {
    bool isMC(!iEvent.isRealData());
    int BTagTh(Jet1BTag);
    float PtTh(Jet1Pt), EtaTh(JetEta);
    std::vector<reco::PFJet> Vect;
    // Declare and open collection
    edm::Handle<std::vector<reco::PFJet> > PFJetsCollection;//check
    iEvent.getByToken(RecoJetToken, PFJetsCollection);//check
    ...
    for(std::vector<reco::PFJet>::const_iterator it=PFJetsCollection->begin(); it!=PFJetsCollection->end(); ++it) {
        ...
        int idx=it-PFJetsCollection->begin();
        //jet.addUserInt("Index", idx);
        //pat::JetRef jetRef(PFJetsCollection, idx);//for QG, not needed yet

	//First of all, jet id selections
        // Quality cut//Later, need to check everything
        //if(JetId==1 && !isLooseJet(jet)) continue;
        //if(JetId==2 && !isTightJet(jet)) continue;
        //if(JetId==3 && !isTightLepVetoJet(jet)) continue;
        // b-tagging
...
/*
reco::PFMET JetAnalyzer::FillRecoMetVector(const edm::Event& iEvent) {
